...
my $GAME_LOG_ARCHIVING = 0;

my $home_doc_dir = File::Spec->catdir(qw( /cygdrive c Users ), $ENV{USER}, 'Documents');

my $ARCHIVE_DIR_DEFAULT = File::Spec->catdir($home_doc_dir, 'asrotor-v2');
$ARCHIVE_DIR_DEFAULT = undef unless -d $ARCHIVE_DIR_DEFAULT;

my $USER_DIR_DEFAULT = File::Spec->catdir($home_doc_dir, 'Paradox Interactive', 'Victoria II');
...
my $autosave_file = File::Spec->catfile($save_dir, 'autosave.v2'); #FIXME
...
unless (-d $gamelog_dir || !$GAME_LOG_ARCHIVING) {
...
while (1) {
	...
	if (defined $st && $st->mtime > $as_mtime) {
		...
		if ($opt_archive) {
			if ($opt_compress) {
				#my $dest_file = "$sdate.ck2.gz"; #FIXME
				my $dest_file = "$sdate.v2.gz";
				...
				#my $dest_file = "$sdate.ck2"; #FIXME
				my $dest_file = "$sdate.v2";
...
sub parse_savegame_date {
	...
	while (<$sf>) {
		...
		next unless /^date="(\d{3,4})\.(\d{1,2})\.(\d{1,2})"/; #FIXME
		$date = [0+$1, 0+$2, 0+$3];
		last;
	}

	if ($line != 1) {
		croak "failed to match savegame date on first line";
	}
